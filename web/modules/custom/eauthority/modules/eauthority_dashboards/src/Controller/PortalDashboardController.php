...
/**
 * @file
 * Contains Drupal\eauthority_dashboards\Controller\PortalDashboardController.
 */

namespace Drupal\eauthority_dashboards\Controller;
...
class PortalDashboardController extends ControllerBase {
  ...
  public function getTasks($highlightQueueID = 0) {
    ...
    foreach($queueIDs as $queueID) {
      ...
      if(   (isset($template->show_details) && $template->show_details) ||
        ...
        $build['task_console_table'][$queueID]['expand'] = array(
          ...
          '#url' => Url::fromRoute('eauthority_dashboards.status_ajax_open', ['processID' => $processID, 'queueID' => $queueID]),
          '#title' => $this->t('Open Details'),
          '#ajax' => array(
            'progress' => [
              'type' => 'throbber',
    ...
    $build['#attached']['library'][] = 'eauthority_dashboards/eauthority_dashboards_css';
    ...
    $thisViewOutput = drupal_render($build);
      
    if($thisViewOutput) {
      $theme_array = [
        '#theme' => 'eauthority_dashboards',
        '#taskconsole' => $thisViewOutput,
      ];
      $build =$theme_array;
    }      
      
    // end new code by gPurro
    ...
    if($canExecute) {
      ...
      if(isset($template->show_details) && $template->show_details) {
              ...
              if($render_build) {
                ...
                if($thisViewOutput) {
                  $task_information_render_array = [
                    '#theme' => 'portal_taskconsole_views',
        ...
        $replace['expand'] = array(
          ...
          '#type' => 'link',
          '#id' => 'maestro-id-ajax-' . $queueID,
          '#url' => Url::fromRoute('eauthority_dashboards.status_ajax_close', ['processID' => $processID, 'queueID' => $queueID]),
          '#title' => $this->t('Close Details'),
          '#ajax' => array(
            'progress' => [
              'type' => 'throbber',
