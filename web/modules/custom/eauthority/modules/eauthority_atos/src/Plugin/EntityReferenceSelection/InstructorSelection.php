 ...
 *   id = "customer:instructor",
 *   label = @Translation("Instructor selection"),
 *   entity_types = {"customer"},
 *   group = "customer",
 *   weight = 1
 * )
 */
class InstructorSelection extends DefaultSelection {
    ...
    protected function buildEntityQuery($match = NULL, $match_operator = 'CONTAINS') {
        ...
        $query = db_select('customer', 'c')->fields('ci', array('instructors_target_id'));
        $query->innerJoin('customer_field_data', 'cfd', "c.id = cfd.id");
        $query->innerJoin('customer__instructors', 'ci', "cfd.id = ci.entity_id");
    ...
    public function getReferenceableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
        ...
        foreach ($result as $key => $record) {
            $item[$record->instructors_target_id] = (float)$record->instructors_target_id;
        }
